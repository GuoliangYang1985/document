...
然而，麦克斯韦方程组预言了真空中光速 $c$ 为一个普适常数，见 @eq-light-speed-c。
...
初始时 $S$ 系和 $S'$ 系的原点重合，$S'$ 系沿 $x$ 轴以速度 $v$ 运动。从$S$ 系到 $S'$ 矩阵形式的洛伦兹变换为 @eq-lorentz-mat。

$$
\mathbf{x}^{'\mu} = \symbf{\Lambda}_{\nu}^{\mu}\mathbf{x}^{v}
$$ {#eq-lorentz-mat}

其中， @eq-lorentz-mat 中的$\symbf{\Lambda}_{\nu}^{\mu}$为洛伦兹变换矩阵 @eq-Lambda-define。
...
将矩阵乘法展开，并代入 @eq-beta，得到 @eq-215834263、 @eq-215834289、 @eq-215834261、 @eq-215834264，合起来就是非矩阵形式的洛伦兹变换 @eq-lorentz。
...
从$S'$系到 $S$ 矩阵形式的逆变换矩阵为 @eq-215835353，其中， @eq-216171626 中的$\symbf{\Lambda}_{\mu}^{\nu}$是 @eq-Lambda-define 中$\symbf{\Lambda}\ _{\nu}^{\mu}$的逆矩阵。
...
电场的势为 @eq-217552139，与磁场的势为 @eq-217552163。
$$\mathbf{E} = - \nabla\phi\mathbf{-}\frac{\partial\mathbf{A}}{\partial t}$$ {#eq-217552139}
$$\mathbf{B} = \nabla\mathbf{\times A}$$ {#eq-217552163}
分量形式为
 $$\mathbf{E}_{\mathbf{i}} = - \frac{\partial\phi}{\partial x^{i}}\mathbf{-}\frac{\partial\mathbf{A}^{i}}{\partial t}$$ {#eq-217553109}
$$\mathbf{B}_{\mathbf{i}} = \left( \nabla\mathbf{\times A} \right)_{i} $$ {#eq-21755310911}                              
...
 时间-空间分量，$\mu = 0$，$\nu = i$，并代入 @eq-217553109，得 @eq-F-0i。

$$
{\mathbf{F}^{0i} = \symbf{\partial}^{0}\mathbf{A}^{i} - \symbf{\partial}^{i}\mathbf{A}^{0}
...
计算$\mathbf{F}^{12}$，代入 @eq-F-0i-coord，得到 @eq-B-field-1。
...
计算$\mathbf{F}^{13}$，代入 @eq-F-0i-coord，得到 @eq-B-field-2。
...
计算$\mathbf{F}^{23}$，代入 @eq-F-0i-coord，得到 @eq-div-partial。
...
根据 @eq-F-tensor 的定义可知$\left( \mathbf{F}^{\mu\nu} \right)^{\top} = - \mathbf{F}^{\nu\mu}$，电磁场张量 $\mathbf{F}^{\mu\nu}$ 是一个反对称二阶张量，可以通过上三角元素对称位置加负号获得下三角的各元素，其矩阵形式为 @eq-inhomo-eq1。
...
$\symbf{\partial}_{\mu}$是协变四维导数 @eq-inhomo-eq3，$\mathbf{J}^{\nu}$ 是四维电流密度 @eq-inhomo-eq2。
...
时间分量当 $\nu = 0$ 时，是$\partial_{\mu}$与$\mathbf{F}^{\mu\nu}$的第0列点乘，为 @eq-inhomo-eq4，其中 $\mathbf{J}^{0} = c\rho$。
...
$$ {#eq-inhomo-eq4}
 ...
 @eq-inhomo-eq4 两边乘以 $c$，得 @eq-inhomo-eq5。
...
利用 @eq-light-speed-c 得 $\mu_{0}c^{2} = 1/\epsilon_{0}$，代入 @eq-inhomo-eq5 整理后得到高斯电场定律 @eq-216076201 。

$$\symbf{\nabla} \mathbf{\cdot E} = \frac{\rho}{\epsilon_{0}}$${#eq-216076201}

#### 安培-麦克斯韦定律

由 @eq-light-speed-c 得 @eq-homo-eq1。

$$
\frac{1}{c^{2}} = \mu_{0}\epsilon_{0}
$$ {#eq-homo-eq1}

空间分量当 $\nu = 1$ 时，是$\symbf{\partial}_{\mu}$与$\mathbf{F}^{\mu\nu}$的第1列点乘，为 @eq-homo-eq2，其中， $\mathbf{J}^{1} = \mathbf{J}_{x}$。
...
将 @eq-homo-eq1 代入 @eq-homo-eq2 后整理得 @eq-216077463，其中，$\frac{\partial\mathbf{B}_{z}}{\partial y} - \frac{\partial\mathbf{B}_{y}}{\partial z} = \left( \symbf{\nabla}\mathbf{\times B} \right)_{x}$。

$$
\left( \symbf{\nabla}\mathbf{\times B} \right)_{x} - \mu_{0}\epsilon_{0}\frac{\partial\mathbf{E}_{x}}{\partial t} = \mu_{0}\mathbf{J}_{x}
$$ {#eq-216077463}

当 $\nu = 2$ 时，是$\symbf{\partial}_{\mu}$与$\mathbf{F}^{\mu\nu}$的第2列点乘，为 @eq-216078264，其中， $\mathbf{J}^{2} = \mathbf{J}_{y}$。
...
将 @eq-homo-eq1 代入 @eq-216078264 后整理得 @eq-216078276，其中，$\frac{\partial\mathbf{B}_{x}}{\partial z} - \frac{\partial\mathbf{B}_{z}}{\partial x} = \left( \symbf{\nabla}\mathbf{\times B} \right)_{y}$。

$$
\left( \symbf{\nabla}\mathbf{\times B} \right)_{y} - \mu_{0}\epsilon_{0}\frac{\partial\mathbf{E}_{y}}{\partial t} = \mu_{0}\mathbf{J}_{y}
$$ {#eq-216078276}

当 $\nu = 3$ 时，是$\symbf{\partial}_{\mu}$与$\mathbf{F}^{\mu\nu}$的第3列点乘，为 @eq-216078498，其中， $\mathbf{J}^{3} = \mathbf{J}_{z}$。
...
将 @eq-homo-eq1 代入 @eq-216078498 后整理得 @eq-216078799，其中，$\frac{\partial\mathbf{B}_{y}}{\partial x} - \frac{\partial\mathbf{B}_{x}}{\partial y} = \left( \symbf{\nabla}\mathbf{\times B} \right)_{z}$。

$$
\left( \symbf{\nabla}\mathbf{\times B} \right)_{z} - \mu_{0}\epsilon_{0}\frac{\partial\mathbf{E}_{z}}{\partial t} = \mu_{0}\mathbf{J}_{z}
$$ {#eq-216078799}

当 $\nu = 1,2,3$ 时，合并三个分量方程 @eq-216077463、 @eq-216078276、 @eq-216078799 为矢量 @eq-215820756，此方程给出安培-麦克斯韦定律。 
...
@tbl-tab-sourceful 中对4维与3维对应关系做了总结，1个4维张量方程包含了2个3维矢量方程。

| 4维形式 | 3维形式 | 物理定律 |
|---|---|---|
| $\partial_{\mu}\mathbf{F}^{\mu\nu} = \mu_{0}\mathbf{J}^{\nu},(\nu = 0)$ | $\nabla \cdot \mathbf{E} = \frac{\rho}{\epsilon_{0}}$ | 高斯电场定律 |
| $\partial_{\mu}\mathbf{F}^{\mu\nu} = \mu_{0}\mathbf{J}^{\nu},(\nu = 1,2,3)$ | $\nabla \times \mathbf{B} = \mu_{0}\mathbf{J} + \mu_{0}\epsilon_{0}\frac{\partial\mathbf{E}}{\partial t}$ | 安培‑麦克斯韦定律 |

: 有源项4维3维对应关系 {#tbl-tab-sourceful}
...
#### 高斯磁场定律
 ...
 @eq-216080650 选择$(\lambda,\mu,\nu) = (1,2,3)$得到 @eq-216081978。

$$
\symbf{\partial}^{1}\mathbf{F}^{23} + \symbf{\partial}^{2}\mathbf{F}^{31} + \symbf{\partial}^{3}\mathbf{F}^{12} = 0
$$ {#eq-216081978}

将 @eq-partial-four 和 @eq-inhomo-eq1 代入 @eq-216081978，得到 @eq-216082498。
...
将 @eq-216082498 整理后得高斯磁场定律 @eq-215821159。
 $$\symbf{\nabla \cdot B} = 0$$ {#eq-215821159}

#### 法拉第电磁感应定律

x-分量 @eq-216080650 选择$(\lambda,\mu,\nu) = (0,2,3)$，得到 @eq-216082919。

$$
\symbf{\partial}^{0}\mathbf{F}^{23} + \symbf{\partial}^{2}\mathbf{F}^{30} + \symbf{\partial}^{3}\mathbf{F}^{02} = 0
$$ {#eq-216082919}

将 @eq-partial-four 和 @eq-inhomo-eq1 代入 @eq-216082919 得到 @eq-216083244。
...
将 @eq-216083244 乘c整理后得 @eq-216083354。

$$
- \frac{\partial\mathbf{B}_{x}}{\partial t} - \frac{\partial\mathbf{E}_{z}}{\partial y} + \frac{\partial\mathbf{E}_{y}}{\partial z} = 0
$$ {#eq-216083354}

 @eq-216083691 中$\frac{\partial\mathbf{E}_{y}}{\partial z} - \frac{\partial\mathbf{E}_{z}}{\partial y} = \left( \nabla \times \mathbf{E} \right)_{x}$，可以得到 @eq-216083691。

$$
\left( \symbf{\nabla}\mathbf{\times E} \right)_{x} = \frac{\partial\mathbf{B}_{x}}{\partial t}
$$ {#eq-216083691}

y-分量 @eq-216080650 选择$(\lambda,\mu,\nu) = (0,3,1)$，得到 @eq-216083842。

$$
\symbf{\partial}^{0}\mathbf{F}^{31} + \symbf{\partial}^{3}\mathbf{F}^{10} + \symbf{\partial}^{1}\mathbf{F}^{03} = 0
$$ {#eq-216083842}

将 @eq-partial-four 和 @eq-inhomo-eq1 代入 @eq-216083842 得到 @eq-216083924。
...
将 @eq-216083924 乘c整理后得 @eq-216084225。

$$
- \frac{\partial\mathbf{B}_{y}}{\partial t} - \frac{\partial\mathbf{E}_{x}}{\partial z} + \frac{\partial\mathbf{E}_{z}}{\partial x} = 0
$$ {#eq-216084225}

 @eq-216084225 中$\frac{\partial\mathbf{E}_{z}}{\partial x} - \frac{\partial\mathbf{E}_{x}}{\partial z} = \left( \symbf{\nabla}\mathbf{\times E} \right)_{y}$，可以得到 @eq-216084334。

$$
\left( \symbf{\nabla}\mathbf{\times E} \right)_{y} = \frac{\partial\mathbf{B}_{y}}{\partial t}
$$ {#eq-216084334}

z-分量 @eq-216080650 选择$(\lambda,\mu,\nu) = (0,1,2)$，得到 @eq-216084498。

$$
\symbf{\partial}^{0}\mathbf{F}^{12} + \symbf{\partial}^{1}\mathbf{F}^{20} + \symbf{\partial}^{2}\mathbf{F}^{01} = 0
$$ {#eq-216084498}

将 @eq-partial-four 和 @eq-inhomo-eq1 代入 @eq-216084498 得到 @eq-216084592。
...
将 @eq-216083244 乘c整理后得 @eq-216084852。

$$
- \frac{\partial\mathbf{B}_{z}}{\partial t} - \frac{\partial\mathbf{E}_{y}}{\partial x} + \frac{\partial\mathbf{E}_{x}}{\partial y} = 0
$$ {#eq-216084852}

 @eq-216084852 中$\frac{\partial\mathbf{E}_{x}}{\partial y} - \frac{\partial\mathbf{E}_{y}}{\partial x} = \left( \symbf{\nabla}\mathbf{\times E} \right)_{z}$，可以得到 @eq-216084874。

$$
\left( \symbf{\nabla}\mathbf{\times E} \right)_{z} = \frac{\partial B_{z}}{\partial t}
$$ {#eq-216084874}

合并三个分量方程 @eq-216083691、 @eq-216084334、 @eq-216084874 为矢量 @eq-215821158 。
$$\symbf{\nabla} \times \mathbf{E} = \frac{\partial\mathbf{B}}{\partial t}$$ {#eq-215821158}

法拉第电磁感应定律 @eq-216085287 有一个负号​，而推导得到的 @eq-215821158 是正号。这个差异源于电磁场张量定义中的符号约定。
$$\symbf{\nabla} \times \mathbf{E} = - \frac{\partial\mathbf{B}}{\partial t}$$ {#eq-216085287}

#### 对应关系

@tbl-tab-sourceless 中对4维与3维对应关系做了总结，1个4维张量方程包含了2个3维矢量方程。

| 4维形式 | 3维形式 | 物理定律 |
|----|--|---|
| $\partial^{\lambda}\mathbf{F}^{\mu\nu} + \partial^{\mu}\mathbf{F}^{\nu\lambda} + \partial^{\nu}\mathbf{F}^{\lambda\mu} = 0$<br>$(\lambda,\mu,\nu) = (1,2,3)$ | $\nabla \cdot \mathbf{B} = 0$ | 高斯磁场定律 |
| $\partial^{\lambda}\mathbf{F}^{\mu\nu} + \partial^{\mu}\mathbf{F}^{\nu\lambda} + \partial^{\nu}\mathbf{F}^{\lambda\mu} = 0$<br>$(\lambda,\mu,\nu) = \left( (0,2,3),(0,3,1),(0,1,2) \right)$ | $\nabla \times \mathbf{E} = - \frac{\partial\mathbf{B}}{\partial t}$ | 法拉第电磁感应定律 |

: 无源项4维3维对应关系 {#tbl-tab-sourceless}

## 协变性与电磁场的变换

证明非齐次麦克斯韦方程组在洛伦兹变换下是协变的，即证明 @eq-216173408。

### 物理量的洛伦兹变换

在洛伦兹变换下，物理量按特定规则变换，二阶电磁场张量 @eq-216170974。$\mathbf{A}^{'\mu}$ 是四维矢量，其变换规则为 @eq-217463243。

$$
\mathbf{A}^{'\mu} = \symbf{\Lambda}_{\nu}^{\mu}\mathbf{A}^{\nu}
$$ {#eq-217463243}

偏导数算符 $\symbf{\partial}^{'\mu}$ 在洛伦兹变换下，坐标微分变换导致偏导数的变换为 @eq-217463483。

$$
\symbf{\partial}^{'\mu} = \symbf{\Lambda}_{\nu}^{\mu}\symbf{\partial}^{v}
$$ {#eq-217463483}

在 $S'$ 系中，电磁场张量定义为 @eq-217463678。

$$
\mathbf{F}^{'\mu\nu} = \symbf{\partial}^{'\mu}\mathbf{A}^{'\nu} - \symbf{\partial}^{'\nu}\mathbf{A}^{'\mu}
$$ {#eq-217463678}

根据 @eq-217463243 和 @eq-217463483 的变换规则，由 @eq-217463678 得 @eq-217464111。
...
洛伦兹变换系数是常数，可提到前面。$\symbf{\Lambda}_{\rho}^{\mu}$和$\symbf{\Lambda}_{\rho}^{\mu}$是洛伦兹变换矩阵的特定元素，即标量实数（或复数），因此，$\symbf{\Lambda}_{\sigma}^{\nu}\symbf{\Lambda}_{\rho}^{\mu} = \symbf{\Lambda}_{\rho}^{\mu}\symbf{\Lambda}_{\sigma}^{v}$。最后，根据 @eq-F-tensor 的定义，可以得到电磁场张量 @eq-216170974。
...
根据坐标变换关系 @eq-lorentz-mat，四维电流密度为 @eq-216171155，四维导数为 @eq-216171177，$\symbf{\Lambda}_{\nu}^{\mu}$为 @eq-Lambda-define。

### 非齐次方程的协变性

将 @eq-216170974 和 @eq-216171177 代入${{\symbf{\partial}'}_{\mu}\mathbf{F}'}^{\mu\nu}$得到 @eq-216171901，系数$\symbf{\Lambda}\ _{\rho}^{\mu}$和$\symbf{\Lambda}\ _{\sigma}^{\nu}$是常数，可以将它们提到微分算符$\symbf{\partial}_{\nu}$的前面。

  $${{\symbf{\partial}'}_{\mu}\mathbf{F}'}^{\mu\nu} = \left( \symbf{\Lambda}_{\mu}^{\nu}\symbf{\partial}_{\nu} \right)\left( \symbf{\Lambda}\ _{\rho}^{\mu}\symbf{\Lambda}\ _{\sigma}^{\nu}\ \mathbf{F}^{\rho\sigma} \right) = \symbf{\Lambda}_{\mu}^{\nu}\symbf{\Lambda}\ _{\rho}^{\mu}\symbf{\Lambda}\ _{\sigma}^{\nu}\ \symbf{\partial}_{\nu}\mathbf{F}^{\rho\sigma}$$   {#eq-216171901}

洛伦兹变换矩阵$满$足正交关系,
${\mathbf{\delta}_{\rho}^{v} = \symbf{\Lambda}}_{\mu}^{\nu}\symbf{\Lambda}\ _{\rho}^{\mu}$，$\nu = \rho$为$\mathbf{\delta}_{\rho}^{v} = 1$，否则为$\mathbf{\delta}_{\rho}^{v} = 0$。

  $${{\symbf{\partial}'}_{\mu}\mathbf{F}'}^{\mu\nu} = \symbf{\Lambda}\ _{\sigma}^{\nu}\ \symbf{\partial}_{\nu}\mathbf{F}^{\rho\sigma}$$ {#eq-216173227}

在 $S$ 系中，根据 @eq-inhomo-eq2 有 $\symbf{\partial}_{\nu}\mathbf{F}^{\sigma\rho} = \mu_{0}\mathbf{J}^{\rho}$ ，代入 @eq-216173227 得 @eq-216173196。

  $${{\symbf{\partial}'}_{\mu}\mathbf{F}'}^{\mu\nu} = \symbf{\Lambda}\ _{\sigma}^{\nu}\ \mu_{0}\mathbf{J}^{\rho} = \mu_{0}\symbf{\Lambda}\ _{\sigma}^{\nu}\ \mathbf{J}^{\rho}$$ {#eq-216173196}

根据四维电流密度的变换 @eq-216171155，有 $\mathbf{J}^{'\nu} = \symbf{\Lambda}\ _{\sigma}^{\nu}\ \mathbf{J}^{\rho}$，得到 @eq-216173408。

$$
{{\symbf{\partial}'}_{\mu}\mathbf{F}'}^{\mu\nu} = \mu_{0}\mathbf{J}^{'\nu}
$$ {#eq-216173408}

 @eq-216173408 和 @eq-inhomo-eq2 有相同的形式，这证明了非齐次麦克斯韦方程组在洛伦兹变换下是协变的。

### 齐次方程的协变性

证明循环形式的齐次方程 @eq-216080650 在洛伦兹变换下形式不变。即，如果在参考系S中上述方程成立，则在参考系
S′中，相应的 @eq-217544542 也成立。

$$
\symbf{\partial}'^{\lambda}\mathbf{F'}^{\mu\nu} + \symbf{\partial}'^{\mu}\mathbf{F}^{'\nu\lambda} + \symbf{\partial}'^{\nu}\mathbf{F}^{'\lambda\mu} = 0
$$ {#eq-217544542}

将变换规则 @eq-217463483 和 @eq-216170974
代入 @eq-217544542 的第一项得 @eq-217544998。
...
同理， @eq-217544542 的第二项得 @eq-217545223，并将哑指标 $(\rho,\alpha,\beta)$ 轮换为 $(\alpha,\beta,\rho)$ 。 @eq-217544542 的第三项得 @eq-217545242，并将哑指标 $(\beta,\rho,\alpha)$ 轮换为 $(\alpha,\beta,\rho)$ 。
...
在每一项中使用了相同的哑指标，可以适当地重命名。 @eq-217544542 变换后的方程左端变为
$$
\begin{aligned}
...
$$    {#eq-217547944}                                                                                                                                                        
由于 $\symbf{\Lambda}\ _{\nu}^{\mu}$
是数字（矩阵元素），数的乘法可交换，因此可以将每个乘积中的因子重新排序。三个乘积实际上包含相同的因子$\symbf{\Lambda}_{\rho}^{\lambda}\mathbf{\ \Lambda}\ _{\alpha}^{\mu}\symbf{\Lambda}\ _{\beta}^{\nu}$，只是顺序不同。根据 @eq-216080650 可得 @eq-217547944 为0，即，证明了 @eq-217544542 成立。

### 电磁场的变换关系

在S\'系中， @eq-216252245 中的$\mathbf{F}^{'\mu\nu}$与 @eq-inhomo-eq1 中$\mathbf{F}^{\mu\nu}$具有相同的形式。
...
根据电磁场张量的变换法则为 @eq-216170974，可以推导出三维空间中电场和磁场分量的显式变换关系。
...
根据 @eq-217378583 得 @eq-217378583。

$$
\gamma^{2}\left( 1 - \beta^{2} \right) = 1
$$ {#eq-217378583}

将 @eq-Lambda-define 和 @eq-inhomo-eq1 和 @eq-217378583 代入 @eq-216254331，只保留非0元素为。
...
$$ {#eq-216258641}
 ...
 @eq-216258641 两边同乘以-c，得到 @eq-216255316。

####  ${\mathbf{E}'}_{y}$与$\mathbf{F}^{\mathbf{'02}}$ 分量

 @eq-216170974 中$\mu = 0$，$\nu = 2$，得 @eq-217378691，由于$\symbf{\Lambda}\ _{\sigma}^{2}$只有在 $\sigma = 2$ 时为
1，其它为 0。
...
将 @eq-Lambda-define 和 @eq-inhomo-eq1 代入 @eq-217378691，只保留非0元素为。

$$
\begin{aligned}
F'^{02} &= \Lambda_{0}^{0} F^{02} + \Lambda_{1}^{0} F^{12} \\
F'^{02} &= \gamma \cdot \left( - \frac{E_y}{c} \right) + (-\gamma\beta) \cdot ( - B_z ) \\
- \frac{E'_y}{c} &= - \gamma \left( \frac{E_y}{c} - \beta B_z \right)
\end{aligned}
$$ {#eq-216258166}

 @eq-216258166 两边同乘以-c，并代入 @eq-216255316，得到 @eq-216256709。

####  ${\mathbf{E}'}_{z}$与$\mathbf{F}^{\mathbf{'03}}$ 分量

 @eq-216170974 中$\mu = 0$，$\nu = 2$，得 @eq-217378932，由于$\symbf{\Lambda}\ _{\sigma}^{3}$只有在 $\sigma = 3$ 时为
1，其它为 0。
...
将 @eq-Lambda-define 和 @eq-inhomo-eq1 中代入 @eq-217378932，只保留非0元素为。
...
$$ {#eq-216258297}
 ...
 @eq-216258297 两边同乘以-c，并代入 @eq-216255316，得到 @eq-216257724。

####  ${\mathbf{B}^{\mathbf{'}}}_{x}$与$\mathbf{F}^{\mathbf{'23}}$ 分量

 @eq-216170974 中$\mu = 0$，$\nu = 2$，得 @eq-217288821，由于$\symbf{\Lambda}\ _{\rho}^{2}$只有在 $\rho = 2$ 时为
...
整理 @eq-217288821 得 @eq-216258492。

####  ${\mathbf{B}'}_{y}$与$\mathbf{F}^{\mathbf{'13}}$ 分量

 @eq-216170974 中$\mu = 1$，$\nu = 3$，得 @eq-217287710，由于$\symbf{\Lambda}\ _{\sigma}^{3}$只有在 $\sigma = 3$ 时为
1，其它为 0。
...
将 @eq-Lambda-define 和 @eq-inhomo-eq1 中代入 @eq-217287710，只保留非0元素为。
...
$$ {#eq-217288259}
 ...
 @eq-beta 代入 @eq-216255316，得到 @eq-216258494。

####  ${\mathbf{B}^{\mathbf{'}}}_{z}$与$\mathbf{F}^{\mathbf{'12}}$ 分量

 @eq-216170974 中$\mu = 1$，$\nu = 2$，得 @eq-217286643，由于$\symbf{\Lambda}\ _{\sigma}^{2}$只有在 $\sigma = 2$ 时为
1，其它为 0。
...
将 @eq-Lambda-define 和 @eq-inhomo-eq1 中代入 @eq-217286643，只保留非0元素为。
$$
\begin{aligned}
\mathbf{F}'^{12}
...
$$ {#eq-217287209}
 ...
 @eq-217287209 两边同乘-1，并将 @eq-beta 代入 @eq-217287209，可以得到 @eq-216258497。

对于沿 $x$ 轴方向的 boost
变换（$S'$ 系相对于 $S$ 系以速度 $v$ 运动），电场分量的变换为 @eq-216255316、 @eq-216256709、 @eq-216257724，磁场分量的变换为 @eq-216258492、 @eq-216258494、 @eq-216258497，其中，垂直于相对运动方向的分量发生混合。
